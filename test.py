...
from yolo.yolo_net import YoloConfig, YoloNet
import numpy as np
from pathlib import Path

import click
from rich.console import Console
from rich.progress import TextColumn, BarColumn, TimeRemainingColumn, TimeElapsedColumn, Progress
...
def test_one_appliance(set_name, app_name, method, info, config):
    ...
    test_loader = DataLoader(test_set, batch_size=config.batch_size, collate_fn=NILMDataset.collate_fn)
    if method == "sl":
    ...
    with torch.no_grad():
        ...
        with Progress(
            ...
            loader_task = progress.add_task(f"{set_name}/{app_name}", total=len(test_loader))
    ...
    return f"{set_name}/{app_name}", test_metric.tp, test_metric.fp, test_metric.fn, pre, rec, f1
